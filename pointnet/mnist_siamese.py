      ...
      if self.train:
         ...
         for i in range(10):
            for j in range(500): # create 500 pairs
               ...
               #adds an array of 3 items to train_data   -
               #1. the selected image
               #2. another image from the same class, a random distance (0,100) away from the selected image
               #3. another image from a different, random class
...
def train(model, device, train_loader, epoch, optimizer):
   ...
   for batch_idx, (data, target) in enumerate(train_loader):
      ...
      optimizer.zero_grad()
      output_positive = model(data[:2]) # return 0 and 1, 0 is the selected image, 1 is another image from the same class
      output_negative = model(data[0:3:2]) # return 0 and 2 (0 to 2 with a step of 2)
